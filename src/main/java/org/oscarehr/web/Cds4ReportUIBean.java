	...
	private SingleMultiAdmissions getAdmissionsSortedSingleMulti() {
		...
		for (CdsClientForm form : cdsForms) {
			...
			else
			{
				...
				// check if the cds form is for a program we're reporting on.
                                if( programIdsToReportOn != null) { // if we've been asked to filter
                                        List<Admission> admissionList = admissionDao.getAdmissions(form.getClientId());
                                        for(Admission ad : admissionList) {
                                                if( programIdsToReportOn.contains(ad.getProgramId())) {
                                                        programIn = true;
                                                }
                                        }
                                        if(!programIn) { // if the program isn't in our allowed list
                                                continue;
                                        }
                                }
			}
